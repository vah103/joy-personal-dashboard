...
export function isIeltsDiagnosticReviewRoute(pathname) { return pathname === REVIEW_PATH; }
...
export async function handleIeltsDiagnosticReviewRequest(request, env) {
  try {
    ...
    if (body.skill !== "writing") return json({ error: "UNSUPPORTED_DIAGNOSTIC_SKILL" }, 400);
    ...
    if (task1Words < 150 || task2Words < 250 || task1Text.length > MAX_ANSWER_CHARS || task2Text.length > MAX_ANSWER_CHARS) {
      return json({ error: "INVALID_WRITING_DIAGNOSTIC", task1Words, task2Words }, 400);
    }
    ...
    const evidenceResult = await env.AI.run(model, { messages: evidenceMessages(submission), temperature: 0.05, max_tokens: 2800 });
    const evidence = parseJsonResult(evidenceResult);
    if (!evidence) return json({ error: "IELTS_EVIDENCE_ANALYSIS_FAILED" }, 502);
    const scoringResult = await env.AI.run(model, { messages: scoringMessages(submission, evidence), temperature: 0.03, max_tokens: 3600 });
    ...
    return json({
      ...
      disclaimer: "Đây là band đầu vào do AI ước lượng, không phải kết quả IELTS chính thức.",
    });
  } catch (error) {
...
- Write every explanation, finding, pattern and uncertainty in clear Vietnamese.
- Keep exact quotations from the candidate in their original English.
- Keep suggested corrected English sentences in English.
- IELTS criterion names and JSON keys must remain exactly as specified below.
- Do not translate or rewrite the candidate's quoted evidence.
...
{
  "task1": {"coverage":["..."],"organisation":["..."],"language":["..."],"evidence":[{"quote":"...","finding":"...","criterion":"..."}]},
  "task2": {"coverage":["..."],"organisation":["..."],"language":["..."],"evidence":[{"quote":"...","finding":"...","criterion":"..."}]},
  "recurringPatterns": [{"category":"...","pattern":"...","evidence":"...","suggestedCorrection":"..."}],
  "uncertainties": ["..."]
}`,
    },
    { role: "user", content: diagnosticSubmissionText(submission) },
  ];
}
...
LANGUAGE REQUIREMENT:
- Write summaries, findings, strength titles, explanations, learning priorities, rewrite instructions and examinerSummary in clear Vietnamese.
- Keep every exact quotation from the candidate in its original English.
- Keep every corrected English sentence in the "correction" field in English.
- Keep the enum values for confidence, category, severity and rewritePlan.task exactly in English as specified.
- Keep JSON keys and IELTS criterion names exactly as specified.
...
{
  "task1": {"scores":{"taskAchievement":0,"coherenceCohesion":0,"lexicalResource":0,"grammaticalRangeAccuracy":0},"summary":"...","evidence":[{"quote":"...","finding":"..."}]},
  "task2": {"scores":{"taskResponse":0,"coherenceCohesion":0,"lexicalResource":0,"grammaticalRangeAccuracy":0},"summary":"...","evidence":[{"quote":"...","finding":"..."}]},
...
function diagnosticSubmissionText(submission) {
  const context = submission.learnerContext ? `LEARNER CONTEXT (do not use this to inflate the score):\n${JSON.stringify(submission.learnerContext)}\n\n` : "";
...
function normalizeWritingReview(value, submission) {
  ...
  if ([...Object.values(task1Scores), ...Object.values(task2Scores)].some((score) => score === null)) return null;
  ...
  return {
    version: 1,
    task1: { scores: task1Scores, band: task1Band, summary: cleanText(value.task1?.summary, 1200), evidence: normalizeEvidence(value.task1?.evidence, 6), words: submission.task1Words, minutes: submission.task1Minutes },
    task2: { scores: task2Scores, band: task2Band, summary: cleanText(value.task2?.summary, 1200), evidence: normalizeEvidence(value.task2?.evidence, 6), words: submission.task2Words, minutes: submission.task2Minutes },
    overallBand,
...
function normalizeEvidence(value, limit) {
  return array(value, limit).map((item) => ({ quote: cleanText(item?.quote, 220), finding: cleanText(item?.finding, 500) })).filter((item) => item.quote || item.finding);
}
function normalizeStrengths(value, limit) {
  return array(value, limit).map((item) => ({ title: cleanText(item?.title, 180), evidence: cleanText(item?.evidence, 240), whyItMatters: cleanText(item?.whyItMatters, 500) })).filter((item) => item.title && item.whyItMatters);
}
function normalizeErrors(value, limit) {
  return array(value, limit).map((item, index) => ({
    code: cleanText(item?.code, 60) || `WR-${index + 1}`,
    category: ["Task", "Coherence", "Vocabulary", "Grammar"].includes(item?.category) ? item.category : "Grammar",
...
function normalizePriorities(value, limit) {
  return array(value, limit).map((item, index) => ({ rank: index + 1, focus: cleanText(item?.focus, 180), reason: cleanText(item?.reason, 600), nextExercise: cleanText(item?.nextExercise, 600) })).filter((item) => item.focus && item.nextExercise);
}
function normalizeRewritePlan(value) {
  const task = ["Task 1", "Task 2", "Both"].includes(value?.task) ? value.task : "Task 2";
  return { task, deadlineHours: boundedNumber(value?.deadlineHours, 48, 12, 168), instructions: array(value?.instructions, 6).map((item) => cleanText(item, 500)).filter(Boolean) };
}
...
function normalizeLearnerContext(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  return { overallTarget: boundedNumber(value.overallTarget, 7, 0, 9), augustWritingTarget: boundedNumber(value.augustWritingTarget, 6, 0, 9), writingMethod: cleanText(value.writingMethod, 300), strengths: cleanText(value.strengths, 800), concerns: cleanText(value.concerns, 800) };
}
function array(value, limit) { return Array.isArray(value) ? value.slice(0, limit) : []; }
function average(values) { return values.reduce((sum, value) => sum + value, 0) / values.length; }
function halfBand(value) { const number = Number(value); return Number.isFinite(number) ? Math.min(9, Math.max(0, Math.round(number * 2) / 2)) : null; }
