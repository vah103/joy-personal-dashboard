...
async function processTaskDeliveries(env) {
  ...
  const rows = await env.DB.prepare(`
    SELECT r.user_email, r.task_id, t.title
  ...
  for (const row of rows.results) {
    ...
    const claimed = await claimTaskDelivery(row.user_email, row.task_id, attemptAt, env);
    if (!claimed) continue;
    ...
    if (!accepted) {
      await releaseTaskDelivery(row.user_email, row.task_id, attemptAt, env);
    }
  }
}

async function claimTaskDelivery(email, taskId, attemptAt, env) {
...
async function releaseTaskDelivery(email, taskId, attemptAt, env) {
  await env.DB.prepare(`
    UPDATE task_reminders
    SET last_notified_at = NULL, updated_at = ?
    WHERE user_email = ? AND task_id = ?
      AND status = 'scheduled' AND last_notified_at = ?
  `).bind(Date.now(), email, taskId, attemptAt).run();
}
...
async function processFocusDeliveries(env) {
  ...
  for (const row of rows.results) {
    const attemptAt = Date.now();
    const claimed = await claimFocusDelivery(row.user_email, attemptAt, env);
    if (!claimed) continue;
    ...
    if (!accepted) {
      await releaseFocusDelivery(row.user_email, attemptAt, env);
    }
  }
}

async function claimFocusDelivery(email, attemptAt, env) {
  const result = await env.DB.prepare(`
...
async function releaseFocusDelivery(email, attemptAt, env) {
  await env.DB.prepare(`
    UPDATE focus_reminders
    SET updated_at = ?
    WHERE user_email = ? AND enabled = 1 AND updated_at = ?
  `).bind(attemptAt - RETRY_AFTER_MS, email, attemptAt).run();
}
  ...
  await Promise.all(rows.results.map(async (row) => {
    try {
      ...
      if (response.ok) accepted += 1;
        ...
        console.error("Joy reminder push failed", response.status, details.slice(0, 300));
      }
    } catch (error) {
      console.error("Joy reminder encryption failed", error);
    }
  }));
