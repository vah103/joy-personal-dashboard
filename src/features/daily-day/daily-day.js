...
(function installDailyDayFeature() {
  ...
  const view = { date: todayKey(), libraryTemplate: "morning", search: "" };
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      .dd-icon{width:38px;height:38px;border:0;border-radius:12px;background:#e7eceb;color:#49616a;font-size:19px}.dd-toolbar{margin-top:18px;display:grid;grid-template-columns:1fr auto;gap:12px}.dd-week{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:8px}
      ...
      @media(max-width:650px){.dd-backdrop{padding:0;align-items:stretch}.dd-shell,.dd-library{width:100%;max-height:100vh;border-radius:0}.dd-scroll{max-height:100vh;padding:16px}.dd-progress{display:none}.dd-toolbar{grid-template-columns:1fr}.dd-week{gap:4px}.dd-week button{padding:0;font-size:8px}.dd-templatebar{grid-template-columns:1fr 1fr}.dd-templatebar label,.dd-select{grid-column:1/-1}.dd-content{display:block}.dd-side{margin-top:12px;grid-template-columns:1fr}.dd-note-card{grid-column:auto}.dd-library-head{display:grid;grid-template-columns:1fr auto}.dd-library-head>div:first-child,.dd-search{grid-column:1/-1}.dd-template-list{grid-template-columns:1fr;max-height:240px;overflow:auto}.dd-timeline-row{grid-template-columns:50px 1fr}.dd-template-items{grid-column:1/-1}}
    `;
  ...
  function renderMain() {
    ...
    root.innerHTML = `<header class="dd-head"><div><h2 class="dd-title" id="dd-title">${esc(t("dailyDay.title"))}</h2><p class="dd-sub">${esc(formatDate(view.date, { weekday: "long", day: "2-digit", month: "2-digit", year: "numeric" }))}</p></div><div class="dd-head-side"><div class="dd-progress"><strong>${esc(t("dailyDay.completedOf", { complete: summary.complete, total: summary.total }))}</strong><div class="dd-track"><i style="width:${percent}%"></i></div></div><button class="dd-icon" type="button" data-dd-close aria-label="${esc(t("common.close"))}">×</button></div></header>
      <div class="dd-toolbar"><div class="dd-week" aria-label="${esc(t("dailyDay.weekNavigation"))}">${week}</div><button class="dd-button" type="button" data-dd-calendar>▣ ${esc(formatDate(view.date, { month: "long", year: "numeric" }))}⌄</button></div>
      <div class="dd-templatebar"><label>${esc(t("dailyDay.defaultDay"))}</label><select class="dd-select" data-dd-select>${options}</select><button class="dd-button" type="button" data-dd-library>↻ ${esc(t("dailyDay.changeTemplate"))}</button><button class="dd-button" type="button" data-dd-library>✎ ${esc(t("dailyDay.editDefaultDay"))}</button></div>
  ...
  function closeMain() { document.querySelector(`#${LIBRARY_ID}`)?.setAttribute("hidden", ""); document.querySelector(`#${MODAL_ID}`)?.setAttribute("hidden", ""); syncBody(); }
  function openLibrary() { installStyles(); ensureMarkup(); view.libraryTemplate = resolvedTemplate(view.date); view.search = ""; renderLibrary(); const modal = document.querySelector(`#${LIBRARY_ID}`); modal.hidden = false; syncBody(); setTimeout(() => modal.querySelector("[data-dd-search]")?.focus(), 0); }
  ...
  document.addEventListener("click", (event) => {
    const day = event.target.closest?.("[data-dd-date]"); if (day) { view.date = day.dataset.ddDate; renderMain(); return; }
    ...
    if (event.target.closest?.("[data-dd-calendar]")) { toast("dailyDay.calendarLater"); return; }
  ...
  document.addEventListener("change", (event) => {
    ...
    const select = event.target.closest?.("[data-dd-select]"); if (select) { setTemplate(view.date, select.value); renderMain(); toast("dailyDay.templateApplied"); }
  });
  ...
  document.addEventListener("mousedown", (event) => { if (event.target.id === MODAL_ID) closeMain(); if (event.target.id === LIBRARY_ID) closeLibrary(); });
  document.addEventListener("keydown", (event) => { if (event.key !== "Escape") return; const library = document.querySelector(`#${LIBRARY_ID}`); if (library && !library.hidden) closeLibrary(); else if (!document.querySelector(`#${MODAL_ID}`)?.hidden) closeMain(); });
