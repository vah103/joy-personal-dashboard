...
function parseLabeledListing(value) {
  ...
  for (const rawLine of normalizeWhitespace(value).split("\n")) {
    ...
    if (labeled) {
      ...
      } else if (fieldValue) appendStructuredField(fields, field, fieldValue);
      continue;
    }
    if (currentSection === "notes") notes.push(line);
    else if (currentSection === "furniture"
      && /\b(?:điện|dien|nước|nuoc|wifi|internet|mạng|mang|dịch vụ|dich vu|gửi xe|gui xe|để xe|de xe|cọc|coc|hợp đồng|hop dong)\b/iu.test(line)) currentSection = "";
    else if (["services", "furniture", "price"].includes(currentSection)) appendStructuredField(fields, currentSection, line);
  }
...
function normalizeServiceValue(key, value) {
  ...
  if (key === "electricity") clean = clean.replace(/\b(\d{1,2}[,.]\d{1,2})(?=\/(?:số|so)(?:\s|$))/giu, "$1k");
...
function availabilityMoment(value) {
  ...
  if (/^(?:vao luon|o ngay|vao o ngay|co the vao o ngay)$/u.test(normalized)) return { key: "now", label: "Vào luôn", summary: "Vào luôn" };
  if (/^(?:dang trong|san phong|phong dang san)$/u.test(normalized)) return { key: "available", label: "Đang trống", summary: "Đang trống" };
...
function buildRoomPresentation(rawInput, rooms, availability) {
  ...
  if (moments.length <= 1) {
    ...
    return { mode: "single", summary: `${rooms.length} phòng${moment?.summary ? ` · ${moment.summary}` : ""}`, priceGroups };
  }
...
function deriveGroupedAvailability(rooms, availability) {
  ...
  if (/^\d{1,2}\/\d{1,2}(?:\/\d{2,4})?$/u.test(detail)) return `${codes.join(", ")} (trống ${detail})`;
  return detail;
}
...
function polishRoomSummary(summary) {
  ...
  return {
    ...summary,
    address, availability, price, roomType, stairs, furniture, services, notes,
