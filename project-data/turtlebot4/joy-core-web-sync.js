(() => {
  ...
  function storeDetail(value) {
    ...
    context().state.joyCoreProject = detail;
  }
  ...
  const canonicalProject = () => detail?.project || null;
  const coreTasks = () => Array.isArray(detail?.tasks) ? detail.tasks : [];
  const coreLogs = () => Array.isArray(detail?.progressLogs) ? detail.progressLogs : [];
  const coreMilestones = () => Array.isArray(detail?.milestones) ? detail.milestones : [];
  const coreEvidence = () => Array.isArray(detail?.evidence) ? detail.evidence : [];
  ...
  const taskStatusLabel = (status) => String(status || "todo").replaceAll("_", " ");
  ...
  function renderCorePanel() {
    ...
    return `<section class="ps-history joy-core-panel" data-joy-core-panel>
      ...
      <div class="joy-core-summary">
        ...
        <article><span>Milestones</span><strong>${coreMilestones().length}</strong></article>
  ...
  async function patchProject(patch, { refresh = true } = {}) {
    if (!canonicalProject()) await loadCore({ refresh: false });
    const current = canonicalProject();
    if (!current) return null;
    const saved = await requestJson(PROJECT_URL, {
      method: "PATCH",
      body: JSON.stringify({ ...patch, baseVersion: Number(current.version || 0) }),
    });
    storeDetail({ ...(detail || {}), project: saved, compatibilityMode: false });
    if (refresh) hubApi.refresh();
    return saved;
  }
  ...
  async function syncProjectFromHub({ force = false, refresh = true } = {}) {
    ...
    const patch = {
      progress,
      currentStageId: ctx.currentStage()?.id || ctx.state.projectState?.project?.currentStageId || null,
  ...
  async function createOrUpdateScheduleTask(input) {
    ...
    storeDetail({ ...(detail || {}), tasks: [task, ...coreTasks().filter((item) => item.id !== task.id)] });
  }
  ...
  async function updateCoreTask(input) {
    ...
    storeDetail({ ...(detail || {}), tasks: coreTasks().map((item) => item.id === taskId ? saved : item) });
  ...
  async function addPlanToJoyCore(button) {
    ...
    try {
      ...
      storeDetail({ ...(detail || {}), tasks: [result.task, ...coreTasks().filter((item) => item.id !== result.task.id)] });
  ...
  document.addEventListener("change", (event) => {
    ...
    if (event.target.closest?.("[data-hub-check],[data-hub-stage-status]")) scheduleProjectSync();
  });
