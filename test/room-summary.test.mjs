...
test("keeps multiline price-room entries and derives availability when Trống is empty", () => {
  const summary = summarizeRoomListing(`
    🌷30% - 12m Mã: 🏆 007B

    🏢Địa chỉ : "Số 54 ngõ 66 Hồ Tùng Mậu - Cầu Giấy

    ⌛️Trống :

    ☘Giá : 4tr5 - P202-502(1/9)
    4tr-p604
    ☘Dạng phòng : studio
    ☘Thang : MÁY
    ...
    - Đóng 1 cọc 1
    - KHÔNG NHẬN XE ĐIỆN
    - PHÒNG KHÔNG CHUNG CHỦ
    - Nguồn hàng cập nhật liên tục tại
      🏆TL21House🏆
  `);

  assert.equal(summary.address, "Số 54 ngõ 66 Hồ Tùng Mậu - Cầu Giấy");
  assert.equal(summary.availability, "P202, P502 (trống 1/9), P604");
  assert.equal(summary.price, "P202, P502: 4tr5/tháng; P604: 4tr/tháng");
  assert.deepEqual(summary.rooms, [
    { title: "Phòng P202", price: "4tr5/tháng", note: "" },
    { title: "Phòng P502", price: "4tr5/tháng", note: "Trống 1/9" },
    { title: "Phòng P604", price: "4tr/tháng", note: "" },
  ]);
  assert.deepEqual(summary.services.map(({ label, value }) => [label, value]), [
    ["Điện", "4k/số"],
    ["Nước", "35k/m³"],
  ...
  assert.deepEqual(summary.notes, ["Đóng 1 cọc 1", "Không nhận xe điện", "Phòng không chung chủ"]);
  assert.doesNotMatch(summary.availability, /đang trống/i);
});
