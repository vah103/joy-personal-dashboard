...
test("room type must be supported by the pasted source", () => {
  ...
  assert.equal(
    normalizeDetectedRoomType("Dạng phòng: STUIDO", "Studio"),
    "Studio",
  );
  assert.equal(
...
test("Room Summary renders room type after the room facts", async () => {
  ...
  assert.match(frontend, /payload\.roomType/);
  assert.match(frontend, /appendRoomType\(details, summary\.roomType\)/);
  assert.match(frontend, /label\.append\("Dạng", " phòng"\)/);
});
