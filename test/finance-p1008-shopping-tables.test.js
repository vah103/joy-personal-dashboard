...
test("canonical dashboard loads the two-table refinement after the shopping module", () => {
  ...
  assert.match(dashboard, /finance-p1008-shopping-tables-v1\.css\?v=joy-finance-p1008-shopping-tables-v1/);
  assert.match(dashboard, /finance-p1008-shopping-tables-v1\.js\?v=joy-finance-p1008-shopping-tables-v2/);
  assert.ok(dashboard.indexOf(baseScript) < dashboard.indexOf(tablesScript));
});
